...
const MoodBoard = () => {
  ...
  const [colorPickerOpen, setColorPickerOpen] = useState([false, false, false]);
        ...
        try {
          if (id !== "undefined" && typeof id !== undefined) {
            ...
            if (response.status === 200) {
              ...
              console.log("Image url value",videoUrlFromRes[videoUrlFromRes.length - 1]);
    ...
    const handleFileUpload = async (event) => {
      ...
      if (file && id && mid) { // Use the 'file' variable instead of 'selectedFile' here
  ...
  const uploadFile = async (e) => {
    ...
    if (files && files.length > 0) { // Check if files array exists and is not empty
  ...
  return (
    <div className="flex flex-col h-screen">
      <div className="flex-auto text-white flex">
        ...
        <div className="p-4 flex flex-col">
          <h1 className="text-2xl w-full text-black font-bold p-2 ">Fonts</h1>
          <div className="p-2 h-full flex justify-center text-black text-3xl">
            SnapSynk Is Awesome
          </div>
      ...
      {/* <div className="flex-1 text-white flex">
          ...
          >
            {false ? (
              ...
              <label htmlFor="file-upload" className="cursor-pointer text-black py-2 px-4 rounded-md">
                <span className="mr-2">+</span> Upload File
              </label>
              <input id="file-upload" type="file" className="hidden" onChange={handleFileUpload} />
              {fileName && <p className="mt-2 text-black">{fileName}</p>}
            </div>
            )}
          </div>
      </div> */}
...
<div className="flex items-center justify-center">
      {selectedFile ? (
        <div>
          <img src={fileDataURL} alt="Uploaded" className="w-800px h-500px" />
          <div className="flex mt-4">
            <button onClick={discardFile} className="bg-red-500 text-white py-2 px-4 rounded-md mr-2">
            ...
            <button onClick={handleFileUpload} className="bg-green-500 text-white py-2 px-4 rounded-md">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 inline-block mr-1" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M9 2a1 1 0 011 1v7h6a1 1 0 110 2h-6v7a1 1 0 01-2 0v-7H4a1 1 0 110-2h6V3a1 1 0 011-1z" clipRule="evenodd" />
              </svg>
              Upload
            </button>
          </div>
        ...
        <div className="relative inline-block">
          <label htmlFor="file-upload" className="cursor-pointer text-black py-2 px-4 rounded-md bg-gray-200 hover:bg-gray-300">
